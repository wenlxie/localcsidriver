...
func TestSetup_ExistingVolumeGroup_UnexpectedTag(t *testing.T) {
	...
	defer clean()
	experr := fmt.Sprintf(
		"Volume group tags did not match expected: err=localvolume: Configured tags don't match existing tags: [] != [%s]",
		tag)
	err = server.Setup()
	if err.Error() != experr {
		t.Fatal(err)
...
func TestSetup_ExistingVolumeGroup_MissingTag(t *testing.T) {
	...
	defer clean()
	experr := fmt.Sprintf(
		"Volume group tags did not match expected: err=localvolume: Configured tags don't match existing tags: [blue] != [some-other-tag]")
	err = server.Setup()
	if err.Error() != experr {
		t.Fatal(err)
	}
