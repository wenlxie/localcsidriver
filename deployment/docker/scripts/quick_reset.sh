...
if [ "$1" == "-h" ]; then
  ...
  echo "The block device must be specified by the environment variable LOCAL_VOL_BLKDEVICE"
  exit 0
fi
...
echo "Calling mkfs"
ionice -c 3 mkfs -F $LOCAL_VOL_BLKDEVICE

echo "Calling wipefs"
ionice -c 3 wipefs -a $LOCAL_VOL_BLKDEVICE
