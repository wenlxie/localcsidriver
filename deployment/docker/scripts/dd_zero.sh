...
if [ "$1" == "-h" ]; then
  ...
  echo "Writes zeros into block device specified by environment variable LOCAL_VOL_BLKDEVICE"
...
function doZero {
  ...
  cmdOut=$(ionice -c 3 dd if=/dev/zero of=$LOCAL_VOL_BLKDEVICE bs=8096 2>&1 | tee /dev/stderr)
